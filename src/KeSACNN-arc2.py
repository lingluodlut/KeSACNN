...
if __name__ == '__main__':
    ...
    rep2 = RepresentationLayer(
            folder + 'word2vec_model/bc_ex3_abner_100.word2vec_model',\
            vec_size=100, frequency=500000)
    rep3 = RepresentationLayer(
            folder + 'word2vec_model/bc_ex3_cui_100.word2vec_model',\
            vec_size=100, frequency=500000)
...
#chanel1    
    ...
    chanel2_input=Input(shape=(s['max_len_abner'],),dtype='int32')
    chanel2_emb=Embedding(rep2.vec_table.shape[0], rep2.vec_table.shape[1], \
                          weights=[rep2.vec_table], input_length=s['max_len_abner'],name='ch2_emb')
    chanel2_vec=chanel2_emb(chanel2_input)

    chanel2_vec=Dropout(0.5)(chanel2_vec)
    chanel2_x=chanel2_vec


    chanel3_input=Input(shape=(s['max_len_cui'],),dtype='int32')
    chanel3_emb=Embedding(rep3.vec_table.shape[0], rep3.vec_table.shape[1], \
                          weights=[rep3.vec_table], input_length=s['max_len_cui'],name='ch3_emb')
    chanel3_vec=chanel3_emb(chanel3_input)

    chanel3_vec=Dropout(0.5)(chanel3_vec)
    chanel3_x=chanel3_vec
    ...
    chanel2_x=conv_block(inputs=chanel2_x, filters=num, window=3, activation='relu',namecb='ch2_conv',name_j=j)
    chanel2_x=MaxPooling1D(pool_size=3,strides=2)(chanel2_x)
    chanel3_x=conv_block(inputs=chanel3_x, filters=num, window=3, activation='relu',namecb='ch3_conv',name_j=j)
    chanel3_x=MaxPooling1D(pool_size=3,strides=2)(chanel3_x)
    j=j+2
    if s['ner_emb']==True and s['cui_emb']==True:
        concat_x=Concatenate(axis=1)([chanel1_x, chanel2_x, chanel3_x])
    elif s['ner_emb']==True and s['cui_emb']==False:
        concat_x=Concatenate(axis=1)([chanel1_x, chanel2_x])
    elif s['ner_emb']==False and s['cui_emb']==True:
        concat_x=Concatenate(axis=1)([chanel1_x, chanel3_x])
    elif s['ner_emb']==False and s['cui_emb']==False:
        concat_x=chanel1_x
        
    concat_x=conv_block(inputs=concat_x, filters=num, window=3, activation='relu',namecb='ch1_conv',name_j=j)
    concat_x=MaxPooling1D(pool_size=3,strides=2)(concat_x)
    j=j+2
    num=num*2
    for i in range(s['block_num']-1):
        if s['self-att']==True:
            concat_x=mulhead_self_att(inputs=concat_x)
        concat_x=conv_block(inputs=concat_x, filters=num, window=3, activation='relu',namecb='ch1_conv',name_j=j)
        concat_x=MaxPooling1D(pool_size=3,strides=2)(concat_x)
        j=j+2
        num=num*2
    ...
    model= Model(inputs=[chanel1_input,chanel2_input,chanel3_input],outputs=predict)
...
#    opt = SGD(lr=0.008, decay=1e-6, momentum=0.9, nesterov=True)
    ...
    result_file='./ppiac-results/resnet-arc2-token-ner-cui-5b-100d.ppiac2_testresult'
    model_file='./ppiac-model/resnet-arc2-token-ner-cui-5b-100d.ppiac2_model'
    ...
    for epoch in xrange(s['epochs']):
        for minibatch in range(batch_num):
            ...
            if each_print==True:
                if minibatch % 1 ==0:
                    dev_res=model.predict([dev_x1,dev_x2,dev_x3],batch_size=100)
                    F=Eval.eval_mulclass(dev_y1, dev_res,False, True)
                    if F[2]>max_f[2]:
                        test_res=model.predict([test_x1,test_x2,test_x3],batch_size=100)
